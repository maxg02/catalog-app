...
type SubmitErrorData = {
    error?: string;
    fieldErrors?: Partial<Record<"name" | "price" | "description" | "details", string>>;
};
...
function AddProduct() {
    ...
    const [createProduct, { isLoading: isCreatingProduct }] = useCreateBusinessProductMutation();
    const {
        control,
        clearErrors,
        handleSubmit,
    ...
    const isSaving = isSubmitting || isCreatingProduct;
    ...
    const getDetailsRecord = (details: ProductDetail[]) => {
        ...
        details.forEach((detail, index) => {
            ...
            if (title && description) {
                detailRecord[title] = description;
            }
        });
    ...
    const onSubmit = async (values: AddProductFormValues) => {
        ...
        try {
            ...
            Object.entries(data?.fieldErrors ?? {}).forEach(([field, message]) => {
                if ((field === "name" || field === "price" || field === "description") && message) {
        ...
        <View className="flex-1">
            ...
            >
                ...
                >
                    ...
                    <Text className="font-jakarta-bold">{isSaving ? "Saving..." : "Save Product"}</Text>
                </Button>
            </Animated.ScrollView>
            ...
            {isSaving && (
                <View className="absolute inset-0 items-center justify-center bg-black/70">
                    ...
                    <Text className="mt-3 font-jakarta-bold text-white">Saving product...</Text>
                </View>
            )}
        </View>
    );
