...
import {
  ...
  getCatalogHref,
} from "@/features/catalog/lib/catalog-products";
import {
...
export default async function CatalogPage({ params, searchParams }: CatalogPageProps) {
  ...
  const featuredProducts = ((featuredProductsResult.data ?? []) as ProductRow[]).map(
    (product) => mapProductRowToDto(product),
  );
  const locationParts = business.location
    ? [business.location.city, business.location.country].filter(Boolean)
    : [];
  const location =
    locationParts.length > 0 ? locationParts.join(", ") : "Location not provided";
